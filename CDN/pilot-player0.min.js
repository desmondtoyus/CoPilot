...
// ad.js
let playList=[];let addclose=document.getElementsByClassName('pilot-video');for(let index=0;index<addclose.length;index++){let c=document.getElementsByClassName('pilot-video')[index];let id=c.childNodes[1].id;let closeBtn=document.createElement('div');closeBtn.innerHTML='&#10006;'
...
function fadeIn(ele){if(parseFloat(ele.style.opacity)<1){ele.style.opacity=parseFloat(ele.style.opacity)+0.005;setTimeout(()=>{fadeIn(ele)},0.5)}};function collapse(ele,playerHeight){let style=window.getComputedStyle(ele);let eleHeight=style.getPropertyValue('height');if(parseInt(eleHeight)<=parseInt(playerHeight)+20){ele.style.height=parseInt(eleHeight)+10+'px';setTimeout(()=>{collapse(ele,playerHeight)},20)}};function fadeOut(ele){if(parseFloat(ele.style.opacity)>=0){ele.style.opacity=parseFloat(ele.style.opacity)-0.05;setTimeout(()=>{fadeOut(ele)},0.5)}};function pause(id){var player=videojs(id);player.ima.pauseAd()}
function play(id){var player=videojs(id);player.ima.resumeAd()}
...
if(endingPlay.parentNode.classList.contains('in_article')||endingPlay.parentNode.classList.contains('in_article_fixed')){endingPlay.parentNode.style.visibility='visible';endingPlay.style.visibility='visible';pause(this.id);let style=window.getComputedStyle(endingPlay);let playerHeight=style.getPropertyValue('height');collapse(endingPlay.parentNode,playerHeight);playList.push(this.id)}}
if(event.type=='pause'){checkScroll()}
if(event.type=='complete'){if(this.inArticle.indexOf('article')>=0){endingPlay.style.display='none';endingPlay.getElementsByClassName('vjs-control-bar')[0].style.display='none';endingPlay.parentNode.style.display='none'}
...
if((!document.getElementById(playId).classList.contains('pilot-checker'))&&(document.getElementById(playId).parentNode.classList.contains('stuck'))){document.getElementById(playId).parentNode.classList.remove('stuck')}
...
if(document.getElementById(playId).classList.contains('in_article_fixed')||document.getElementById(playId).classList.contains('standard')){document.getElementById(playId).classList.remove("pilot-checker");pause(playId)}}}}}
